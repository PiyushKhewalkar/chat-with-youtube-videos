...
export const launchSession = async (req, res) => {
  try {
    ...
    if (!videoUrl)
      return res.status(500).json({ message: "Video URL missing" });
    ...
    return res
      .status(201)
      .json({ message: "Session launched succesfully", session });
  } catch (error) {
    return res
      .status(500)
      .json({ error: "Internal Server Error", details: error.message });
...
export const askQuestion = async (req, res) => {
  try {
    ...
    if (!userInput)
      return res.status(500).json({ message: "user input is required" });
